 ...
 * @extends Behavior<T>
 */
class Action_Filter extends Behavior
{
    /**
     ...
     */
    public function before_filter($event): void
    {
        if (!$this->is_active($event->action)) {
            return;
        }
        $event->is_valid = $this->before_action($event->action);
        if ($event->is_valid) {
     ...
     * @param ActionEvent $event
     */
    public function after_filter($event): void
    {
        $event->result = $this->after_action($event->action, $event->result);
        $this->owner->off(Controller::EVENT_AFTER_ACTION, [$this, 'afterFilter']);
    }
    /**
     ...
     * @since 2.0.7
     */
    protected function get_action_id($action)
    {
        if ($this->owner instanceof Module) {
            $mid = $this->owner->get_unique_id();
            $id = $action->get_unique_id();
     ...
     */
    protected function is_active($action): bool
    {
        ...
        return !$except_match && $only_match;
    }
}