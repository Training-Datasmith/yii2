...
use yii\data\Array_Data_Provider;
 ...
 * @since 2.0
 */
class Data_Column extends Column
{
    /**
     ...
     * @see \yii\helpers\Html::renderTagAttributes() for details on how attributes are being rendered.
     */
    public $filter_input_options = ['class' => 'form-control', 'id' => null];
    /**
    ...
    public function init(): void
    {
        ...
        if ($this->filter_attribute === null) {
            $this->filter_attribute = $this->attribute;
        }
    }
    /**
     * {@inheritdoc}
    ...
    protected function render_header_cell_content()
    {
        ...
        $label = $this->get_header_cell_label();
        if ($this->encode_label) {
            $label = Html::encode($label);
        }
        if ($this->attribute !== null && $this->enable_sorting && ($sort = $this->grid->data_provider->get_sort()) !== false && $sort->has_attribute($this->attribute)) {
            return $sort->link($this->attribute, array_merge($this->sort_link_options, ['label' => $label]));
        }
        return $label;
    }
    /**
    ...
    {
        $provider = $this->grid->data_provider;
        if ($this->label === null) {
            if ($this->attribute === null) {
                $label = '';
            } elseif ($provider instanceof Active_Data_Provider && $provider->query instanceof Active_Query_Interface) {
                /** @var Model $modelClass */
                $model_class = $provider->query->model_class;
                $model = $model_class::instance();
                $label = $model->get_attribute_label($this->attribute);
            } elseif ($provider instanceof Array_Data_Provider && $provider->model_class !== null) {
                /** @var Model $modelClass */
                $model_class = $provider->model_class;
                $model = $model_class::instance();
                $label = $model->get_attribute_label($this->attribute);
            } elseif ($this->grid->filter_model !== null && $this->grid->filter_model instanceof Model) {
                $label = $this->grid->filter_model->get_attribute_label($this->filter_attribute);
            } else {
    ...
    protected function render_filter_cell_content()
    {
        ...
        $model = $this->grid->filter_model;
        if ($this->filter !== false && $model instanceof Model && $this->filter_attribute !== null && $model->is_attribute_active($this->filter_attribute)) {
            if ($model->has_errors($this->filter_attribute)) {
                Html::add_css_class($this->filter_options, 'has-error');
                $error = ' ' . Html::error($model, $this->filter_attribute, $this->grid->filter_error_options);
            } else {
                $error = '';
            }
            if (is_array($this->filter)) {
                $options = array_merge(['prompt' => '', 'strict' => true], $this->filter_input_options);
                return Html::active_drop_down_list($model, $this->filter_attribute, $this->filter, $options) . $error;
            }
            if ($this->format === 'boolean') {
                $options = array_merge(['prompt' => '', 'strict' => true], $this->filter_input_options);
                return Html::active_drop_down_list($model, $this->filter_attribute, [1 => $this->grid->formatter->boolean_format[1], 0 => $this->grid->formatter->boolean_format[0]], $options) . $error;
            }
            $options = array_merge(['maxlength' => true], $this->filter_input_options);
            return Html::active_text_input($model, $this->filter_attribute, $options) . $error;
        }
        return parent::render_filter_cell_content();
    }
    /**
     ...
     */
    public function get_data_cell_value($model, $key, $index)
    {
        if ($this->value !== null) {
            if (is_string($this->value)) {
                return Array_Helper::get_value($model, $this->value);
            }
            return call_user_func($this->value, $model, $key, $index, $this);
        }
        if ($this->attribute !== null) {
            return Array_Helper::get_value($model, $this->attribute);
        }
        return null;
    }
    /**
    ...
    protected function render_data_cell_content($model, $key, $index)
    {
        if ($this->content === null) {
            return $this->grid->formatter->format($this->get_data_cell_value($model, $key, $index), $this->format);
        }
