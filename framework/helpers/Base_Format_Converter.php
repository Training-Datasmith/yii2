...
class Base_Format_Converter
{
     ...
     */
    public static $php_fallback_date_patterns = ['short' => ['date' => 'n/j/y', 'time' => 'H:i', 'datetime' => 'n/j/y H:i'], 'medium' => ['date' => 'M j, Y', 'time' => 'g:i:s A', 'datetime' => 'M j, Y g:i:s A'], 'long' => ['date' => 'F j, Y', 'time' => 'g:i:sA', 'datetime' => 'F j, Y g:i:sA'], 'full' => ['date' => 'l, F j, Y', 'time' => 'g:i:sA T', 'datetime' => 'l, F j, Y g:i:sA T']];
    /**
     * @var array the jQuery UI fallback definition to use for the ICU short patterns `short`, `medium`, `long` and `full`.
     * This is used as fallback when the `intl` extension is not installed.
     */
    public static $jui_fallback_date_patterns = ['short' => ['date' => 'd/m/y', 'time' => '', 'datetime' => 'd/m/y'], 'medium' => ['date' => 'M d, yy', 'time' => '', 'datetime' => 'M d, yy'], 'long' => ['date' => 'MM d, yy', 'time' => '', 'datetime' => 'MM d, yy'], 'full' => ['date' => 'DD, MM d, yy', 'time' => '', 'datetime' => 'DD, MM d, yy']];
    private static array $_icu_short_formats = [
        'short' => 3,
     ...
     * @throws \Exception
     */
    public static function convert_date_icu_to_php($pattern, $type = 'date', $locale = null)
    {
        if (isset(self::$_icu_short_formats[$pattern])) {
            if (extension_loaded('intl')) {
                $pattern = self::create_formatter($locale, $type, $pattern);
            } else {
                return static::$php_fallback_date_patterns[$pattern][$type];
            }
        }
        ...
        return strtr($pattern, array_merge($escaped, [
            ...
            // Timezone abbreviation
            'zzzz' => 'T',
            // Timezone full name, not supported by php but we fallback
            'Z' => 'O',
     ...
     * @return string The converted date format pattern.
     */
    public static function convert_date_php_to_icu($pattern): string
    {
        ...
        $result = strtr($pattern, [
            ...
            '\n' => "'n'",
            'n' => 'M',
            // Numeric representation of a month, without leading zeros — 1 through 12, not supported by ICU but we fallback to "with leading zero"
            '\t' => "'t'",
            't' => '',
     ...
     * @throws \Exception
     */
    public static function convert_date_icu_to_jui($pattern, $type = 'date', $locale = null)
    {
        if (isset(self::$_icu_short_formats[$pattern])) {
            if (extension_loaded('intl')) {
                $pattern = self::create_formatter($locale, $type, $pattern);
            } else {
                return static::$jui_fallback_date_patterns[$pattern][$type];
            }
        }
        // https://unicode-org.github.io/icu/userguide/format_parse/datetime/#datetime-format-syntax
        // escaped text
     ...
     * @return string The converted date format pattern.
     */
    public static function convert_date_php_to_jui($pattern): string
    {
        // https://www.php.net/manual/en/function.date
        return strtr($pattern, [
            ...
            // Full Date/Time
            'c' => 'yyyy-MM-dd',
            // ISO 8601 date, e.g. 2004-02-12T15:19:21+00:00, skipping the time here because it is not supported
            'r' => 'D, d M yy',
            // RFC 2822 formatted date, Example: Thu, 21 Dec 2000 16:01:07 +0200, skipping the time here because it is not supported
            'U' => '@',
        ]);
    }
    /**
     ...
     */
    private static function create_formatter($locale, $type, $pattern)
    {
        if ($locale === null) {
            $locale = Yii::$app->language;
        }
        if ($type === 'date') {
            $formatter = new Intl_Date_Formatter($locale, self::$_icu_short_formats[$pattern], Intl_Date_Formatter::NONE);
        } elseif ($type === 'time') {
            $formatter = new Intl_Date_Formatter($locale, Intl_Date_Formatter::NONE, self::$_icu_short_formats[$pattern]);
        } else {
            $formatter = new Intl_Date_Formatter($locale, self::$_icu_short_formats[$pattern], self::$_icu_short_formats[$pattern]);
        }
        return $formatter->get_pattern();
    }
}