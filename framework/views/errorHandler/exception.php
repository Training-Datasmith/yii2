...
if ($exception instanceof \yii\web\Http_Exception) {
    echo (int) $exception->status_code . ' ' . $handler->html_encode($name);
} else if ($name !== null) {
    echo $handler->html_encode($name . ' – ' . get_class($exception));
} else {
...
<body>
    ...
    <div class="header">
        <div class="tools">
            <textarea id="clipboard"><?php 
echo $handler->html_encode($exception);
?></textarea>
            ...
            <a href="https://stackoverflow.com/search?<?php 
echo http_build_query(['q' => $exception->get_message()]);
?>" title="Search error on Stackoverflow" target="_blank">
            ...
            <a href="https://www.google.com/search?<?php 
echo http_build_query(['q' => $exception->get_message()]);
...
if ($exception instanceof \yii\base\ErrorException) {
    ?>
            ...
            <h1>
                <span><?php 
    echo $handler->html_encode($exception->get_name());
    ?></span>
                &ndash; <?php 
    ...
    ?>
            ...
            <h1><?php 
    if ($exception instanceof \yii\web\Http_Exception) {
        echo '<span>' . $handler->create_http_status_link($exception->status_code, $handler->html_encode($exception->get_name())) . '</span>';
        echo ' &ndash; ' . $handler->add_type_links(get_class($exception));
    } else {
        $name = $handler->get_exception_name($exception);
        if ($name !== null) {
            echo '<span>' . $handler->html_encode($name) . '</span>';
            echo ' &ndash; ' . $handler->add_type_links(get_class($exception));
        } else {
            echo '<span>' . $handler->html_encode(get_class($exception)) . '</span>';
        }
    }
    ?></h1>
        <?php 
...
?>
    ...
    <div class="call-stack">
        <?php 
echo $handler->render_call_stack($exception);
?>
    </div>
...
?></p>
        <p><?php 
echo $handler->create_server_information_link();
?></p>
        <p><a href="https://yiiframework.com/">Yii Framework</a>/<?php 
echo $handler->create_framework_version_link();
?></p>
    </div>
